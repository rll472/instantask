...
export default function Portal() {
  ...
  const handleUnsubscribe = async () => {
    ...
    try {
      ...
      const data = await response.json();
      setMessage(response.ok ? (data.message || "Subscription cancelled.") : (data.error || "Unsubscribe failed."));
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        <title>Instantask - Portal</title>
        <meta name="description" content="Manage your Instantask agents and billing." />
      ...
      <main>
        <section className="py-16 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-4xl font-extrabold text-gray-900 text-center">Your Portal</h2>
            <p className="mt-4 text-lg text-gray-600 text-center max-w-2xl mx-auto">
              Manage your agents and billing details.
            </p>
            ...
            {user ? (
              <div className="mt-12 max-w-lg mx-auto bg-white p-8 rounded-lg shadow-md">
                ...
                <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                  <p className="text-sm text-gray-700 font-medium">Active Agents</p>
                  <p className="text-3xl font-extrabold text-blue-600">{user.agents_count}</p>
                </div>
