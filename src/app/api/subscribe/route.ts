...
function getPricePerAgent(count: number): number {
  if (count >= 5) return 199_00; // cents
  if (count >= 2) return 249_00;
  return 299_00;
}

export async function POST(request: Request) {
  const { name, companyName, address, phone, email, password, agentsCount } = await request.json();
  ...
  const count = Math.max(1, parseInt(agentsCount) || 1);
  const pricePerAgent = getPricePerAgent(count);
  const totalCents = pricePerAgent * count;
  ...
  try {
    ...
    const agentLabel = count === 1 ? '1 Agent' : `${count} Agents`;
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'usd',
          product_data: {
            name: `Instantask — ${agentLabel}`,
            description: `${agentLabel} at $${pricePerAgent / 100}/agent/month`,
          },
